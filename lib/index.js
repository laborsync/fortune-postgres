...
const idCharset = [idDigits, idLetters, idLetters.toLowerCase()].join('');
...
module.exports = Adapter => class PostgresAdapter extends Adapter {
    ...
    _computeSelect(type, options) {
        ...
        const selects = fields.map((field, index) => {
            return '"' + field + (index && recordTypes[type][field].type === String ? '"::text' : '"' + (!index ? ' AS id' : ''));
        });
    ...
    update(type, updates, meta) {
        ...
        return Promise.all(updates.map((update, index) => {
                ...
                for (const field in update.push) {
                    ...
                    const add = 'ARRAY_CAT("' + field + '", $' + params.length + ')';

                    params.push(update.id);
                    sets.push('"' + field + '" = (SELECT ARRAY(SELECT DISTINCT UNNEST(' + add + ')) FROM ' + this._computeTable(type) + ' WHERE "' + this._computeTypeId(type) + '" = $' + params.length + ')');
                ...
                for (const field in update.pull) {
                    ...
                    if (update.pull[field].length) {
                        ...
                        params.push(update.id);
                        sets.push('"' + field + '" = (SELECT ARRAY(SELECT DISTINCT UNNEST(' + remove + ')) FROM ' + this._computeTable(type) + ' WHERE "' + this._computeTypeId(type) + '" = $' + params.length + ')');
    ...
    find(type, ids, options, meta) {
        ...
        return client.query(sql, params)
            .then((results) => {
                ...
                records.count = results.rows.length;
