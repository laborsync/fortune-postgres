...
module.exports = Adapter => class PostgresAdapter extends Adapter {
    ...
    _computeSelect(type, options) {
        ...
        const selects = fields.map((field) => {
            return '"' + field + '"' + (recordTypes[type][field].type === String ? '::text' : '');
        });

        const typeId = this._computeTypeId(type);
        if (fields.indexOf(typeId) < 0) {
            selects.unshift('"' + typeId + '"');
        }
        ...
        return Promise.all(promises)
            .then(([now]) => {
                ...
                return Promise.all(updates.map((update, index) => {
                        ...
                        if (now) {
                            updates[index].replace.updated = now;
                            params.push(now);
                            sets.push('"updated" = $' + params.length)
                        }
    ...
    find(type, ids, options, meta) {
        ...
        return Promise.all([
            ...
            .then(([
                ...
                records.count = Number(countResults.rowCount);
            ...
            .then((client) => {
                ...
                Object.assign(scope, this, {
                    ...
                    endTransaction(error) {
                        console.log(error ? 'ROLLBACK' : 'COMMIT');
                        console.log('\n');
                        return client.query(error ? 'ROLLBACK' : 'COMMIT')
                            .then(() => {
                                client.release();
                            });
                    }
    ...
    _queryNow() {
        ...
        const nowQuery = "SELECT TIMEZONE('UTC', NOW())::text AS now";

        return client.query(nowQuery)
            .then((nowResults) => {
                return nowResults.rows[0].now;
            });
