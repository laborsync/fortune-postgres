...
module.exports = Adapter => class PostgresAdapter extends Adapter {
    ...
    _computeSelect(type, options) {
        ...
        let fields = Object.keys(options.fields).filter((field) => {
            return recordTypes[type][field] && !(denormalizedInverseKey in recordTypes[type][field]);
        });
    ...
    _computeWhere(type, ids, options, params, root = true) {

        const where = this._computeWhereLogicalOperators(type, options, params, [
            this._computeWhereIds(type, ids, params),
            this._computeWhereExists(type, options, params),
            this._computeWhereMatch(type, options, params),
            this._computeWhereRange(type, options, params),
    ...
    _computeWhereExists(type, options, params) {
        ...
        return whereExists.length ? this._computeWhereLogicalOperators(type, options.range, params, whereExists.join(' AND ')) : null;

    }

    _computeWhereMatch(type, options, params) {
        ...
        return whereMatch.length ? this._computeWhereLogicalOperators(type, options.match, params, whereMatch.join(' AND ')) : null;

    }

    _computeWhereRange(type, options, params) {
        ...
        return whereRange.length ? this._computeWhereLogicalOperators(type, options.range, params, whereRange.join(' AND ')) : null;

    }

    _computeWhereLogicalOperators(type, options, params, where) {
        ...
        logicalOperators.map((logicalOperator) => {
            ...
            if (options[logicalOperator]) {
                where = [
                    where,
                    this._computeWhere(type, undefined, options[logicalOperator], params, false)
                ].filter(Boolean);
    ...
    _computeQuery(type, ids, options) {
        ...
        const whereSql = this._computeWhere(type, ids, options, params);
    ...
    find(type, ids, options, meta) {
        ...
        options = options || {};
        options.query = options.query || this._computeQuery(type, ids, options);

        return client.query(options.query.sql, options.query.params)
